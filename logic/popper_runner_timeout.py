...
def run_popper(depth: int, depth_folder: str, timeout: int):
    # Run Popper
    settings = Settings(kbpath=depth_folder, max_vars=6, max_body=8, timeout=int(timeout))
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python popper_runner.py <depth:int> <depth_folder:path> <timeout: timeout>")
        sys.exit(1)

    depth_arg = int(sys.argv[1])
    folder_arg = sys.argv[2]
    timeout_arg = sys.argv[3]

    run_popper(depth_arg, folder_arg, timeout_arg)
