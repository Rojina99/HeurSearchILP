import os
import argparse
from popper.util import Settings, order_prog, format_literal
from popper.loop import learn_solution

def format_rule(rule, depth, rule_idx):
    head, body = rule
    head_str = ''
    if head:
        head_str = format_literal(head)
    body_str = ','.join(format_literal(literal) for literal in body)
    return f'd{depth}_r{rule_idx}_{head_str}:- {body_str}.'
...
def write_prog_on_file(settings, depth, prog, score, filename="results.txt"):
    with open(filename, "a") as f:
        ...
        for idx, rule in enumerate(order_prog(prog)):
            f.write(format_rule(settings.order_rule(rule), depth, idx)+'\n')
        f.write('*'*30+'\n\n')
