        ...
        if rules_file and os.path.exists(rules_file) and os.path.getsize(rules_file) > 0:
            try:
                ...
                if processed_rules:
                    ...
                    print(
                        f"    PureILPHeuristic: No rules found in {rules_file} after processing by load_and_process_clause_to_get_unique_clause.")
...
def format_learned_rule_for_bk(rule_str: str, new_head_predicate: str) -> str:
    ...
    if ":-" in rule_str:
        ...
        new_head = re.sub(r"^\s*(f|clause|goal)\s*\(\s*([A-Za-z_0-9]+)\s*\)", rf"{new_head_predicate}(\2)",
                          head.strip())
        ...
        new_head = re.sub(r"^\s*(f|clause|goal)\s*\(\s*([A-Za-z_0_9]+)\s*\)", rf"{new_head_predicate}(\2)",
                          rule_str.strip())
        formatted_rule = f"{new_head}."
...
def prepare_popper_input_files(
    ...
    with open(target_bk_path, 'w') as f:
        f.write(bk_content)

    bias_content = ""
    if os.path.exists(base_bias_path):
        with open(base_bias_path, 'r') as f:
            bias_content += f.read()
    else:
...
def astar_example_generation_worker(  # Accepts max_astar_steps_per_instance
    ...
    for i, instance in enumerate(astar_search.instances):
        ...
        if instance.goal_node:
            ...
            if path_cost >= target_path_cost:
                local_positive_examples.append(original_start_state)
            elif path_cost < target_path_cost:
...
def run_astar_for_example_generation_parallel(
    ...
    while attempt < max_attempts:
        ...
        if attempt > 1:  # This is a retry
            print(
                f"    [A* Gen Parallel Retry Attempt {attempt - 1}] No positive examples found. Increasing A* expansions.")
            ...
            print(
                f"    [A* Gen Parallel Retry Attempt {attempt - 1}] New max_astar_expansions_per_instance: {current_max_astar_expansions}")
        ...
        print(
            f"  [A* Gen Parallel] Target Cost: {target_path_cost}. States: ~{num_states_for_astar_run}. Workers: {num_workers}. A* Weight: {astar_weight}. Max Expansions: {current_max_astar_expansions}.")
        ...
        if scramble_depths_list:
            print(
                f"    [A* Gen Parallel] Scramble depths: min={min(scramble_depths_list) if scramble_depths_list else 'N/A'}, "
        ...
        for _ in range(active_workers):  # Iterate based on active workers
            ...
            try:
                ...
                worker_id, pos_ex, neg_ex, worker_expansions_limit = result_queue.get(
                    timeout=timeout_time)  # 4 hour timeout
                ...
                print(
                    f"    [A* Gen Parallel] Worker {worker_id} finished (used limit {worker_expansions_limit}), found Pos={len(pos_ex)}, Neg={len(neg_ex)}")
            except Exception as e:
                print(
                    f"    [A* Gen Parallel] Worker result retrieval error or timeout: {e} with timeout time {timeout_time}")

        for p in processes:
            p.join(timeout=60)
            if p.is_alive():
                print(f"    [A* Gen Parallel] Worker process {p.pid} did not terminate, killing.");
                p.terminate();
                p.join()
        ...
        print(
            f"    [A* Gen Parallel] Classification Complete (Attempt {attempt}): Total Unique Positives={len(overall_positive_examples)}, Total Unique Negatives={len(overall_negative_examples)}")
...
def save_main_checkpoint(filepath: str, current_depth_to_learn: int, all_accumulated_rules: Dict[int, List[str]],
    ...
    print(
        f"  Main checkpoint saved to {filepath} (next depth: {current_depth_to_learn + 1}, max_exp: {current_max_astar_expansions})")
            ...
            if not sorted_depths:
                ...
                for depth in sorted_depths:
                    ...
                    if rules:
                        ...
                        f.write("(No rules learned for this depth)\n")
                    f.write("\n")
            f.write("=" * 40 + "\n")
        print(f"  Formatted learned rules saved to {filepath}")
    except Exception as e:
        print(f"Error saving formatted rules to {filepath}: {e}")
...
def main():
    ...
    parser.add_argument('--learned_program_dir', type=str, required=True,
                        help="Directory containing template bk.pl and bias.pl for Popper.")
    parser.add_argument('--popper_runner_path', type=str, default='logic/popper_runner.py',
                        help="Path to popper_runner.py utility.")
    parser.add_argument('--max_depth_to_learn', type=int, default=31,
                        help="Maximum lower bound depth to learn rules for.")
    parser.add_argument('--states_per_astar_total', type=int, default=DEFAULT_STATES_PER_DEPTH_TOTAL_FOR_ILP,
                        help="Total states for A* example generation for one depth's ILP refinement.")
    parser.add_argument('--max_scramble_steps', type=int, default=60,
                        help="Max random walk steps from goal for generating A* start states.")
    parser.add_argument('--astar_heuristic_batch_size', type=int, default=ASTAR_HEURISTIC_CALL_BATCH_SIZE,
                        help="Batch size for A* when calling heuristic function.")
    parser.add_argument('--ilp_convergence_iterations', type=int, default=ILP_CONVERGENCE_ITERATIONS,
                        help="Max iterations for ILP rule refinement for a single depth.")
    parser.add_argument('--max_astar_expansions_per_instance', type=int, default=20000,
                        help="Initial maximum A* node expansions for a single problem instance.")  # Renamed to 'initial'
    parser.add_argument('--num_astar_workers', type=int, default=NUM_PARALLEL_ASTAR_WORKERS,
                        help="Number of parallel workers for A* example generation.")
    parser.add_argument('--astar_weight', type=float, default=DEFAULT_ASTAR_WEIGHT,
                        help="Weight for Weighted A* search in example generation.")
    parser.add_argument('--astar_retry_multiplier', type=float, default=DEFAULT_MAX_ASTAR_EXPANSIONS_RETRY_MULTIPLIER,
                        help="Multiplier for max_astar_expansions on retry if no positive examples.")
    parser.add_argument('--astar_retry_increment', type=int, default=DEFAULT_MAX_ASTAR_EXPANSION_INCREMENT_ON_RETRY,
                        help="Fixed increment for max_astar_expansions on retry.")
    ...
    args.model_dir = os.path.join(args.model_dir,
                                  f'env_{args.env}_pdir_{args.learned_program_dir.strip("/")}_depth_{args.max_depth_to_learn}_'
                                  ...
                                  f'workers_{args.num_astar_workers}_rm_{args.astar_retry_multiplier}_ri_{args.astar_retry_increment}')
    ...
    heuristic_fn_wrapper = PureILPHeuristic(env, popper_run_base_dir=popper_run_base_dir,
                                            initial_rules_file=master_ilp_rules_file)

    # Load checkpoint, including the potentially adapted max_astar_expansions
    start_depth_main_loop, all_accumulated_rules_loaded, loaded_max_astar_expansions = load_main_checkpoint(
        main_checkpoint_file, args.max_astar_expansions_per_instance)
    ...
    if all_accumulated_rules:
        ...
        print(
            f"Resuming. Loaded {len(all_accumulated_rules)} depths of rules from checkpoint. Next depth to learn: {start_depth_main_loop}")
    else:
    ...
    for current_depth_to_learn in range(start_depth_main_loop, args.max_depth_to_learn + 1):
        ...
        if states_per_refinement_iter < 10 and args.states_per_astar_total > 0: states_per_refinement_iter = max(10,
                                                                                                                 args.states_per_astar_total)
        ...
        for ilp_refinement_iter in range(args.ilp_convergence_iterations):
            ...
            print(
                f"  [ILP Refine iter {ilp_refinement_iter + 1}/{args.ilp_convergence_iterations} for depth {current_depth_to_learn}]")
            ...
            if updated_max_expansions_after_run > current_max_astar_expansions:
                print(
                    f"    [Main Loop] max_astar_expansions_per_instance updated from {current_max_astar_expansions} to {updated_max_expansions_after_run} due to retry.")
            ...
            print(
                f"    Total unique examples for depth {current_depth_to_learn} after A*: Pos={len(accumulated_pos_examples_this_depth)}, Neg={len(accumulated_neg_examples_this_depth)}")
            ...
            if not accumulated_pos_examples_this_depth and ilp_refinement_iter == 0:  # No positive examples even after potential retry on first iteration
                print(
                    f"    [Warning] No positive examples generated for depth {current_depth_to_learn}. Popper may not find rules.")
            ...
            if accumulated_pos_examples_this_depth or accumulated_neg_examples_this_depth:  # Run Popper if any examples
                ...
                print("    No positive or negative examples to run Popper.")
            ...
            if current_rules_for_this_depth_iter:
                ...
                print(f"    Popper found no rules in this iteration.")
        ...
        if final_learned_clauses_for_this_depth:
            if current_depth_to_learn not in all_accumulated_rules or \
                    ClauseSet(final_learned_clauses_for_this_depth) != ClauseSet(
                all_accumulated_rules.get(current_depth_to_learn, [])):
                rules_changed_this_depth = True
                all_accumulated_rules[current_depth_to_learn] = final_learned_clauses_for_this_depth
                print(
                    f"  Final rules for depth {current_depth_to_learn} (count: {len(final_learned_clauses_for_this_depth)}).")
            else:
                print(f"  No change to rules for depth {current_depth_to_learn} compared to master list.")
            ...
            print(f"  No rules finalized for depth {current_depth_to_learn}.")

        if rules_changed_this_depth or (
                not os.path.exists(master_ilp_rules_file) and final_learned_clauses_for_this_depth):
            save_rules_to_file(master_ilp_rules_file, all_accumulated_rules)
            heuristic_fn_wrapper.load_rules(master_ilp_rules_file)
